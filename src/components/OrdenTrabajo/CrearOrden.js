...
class OrdenTrabajo extends React.Component {
    constructor() {
        ...
        // this.agregarOcultar = this.agregarOcultar.bind(this);
        // this.removerOcultar = this.removerOcultar.bind(this);
    }
    ...
    imprimir() {
        ...
        console.log(this.state.estadoImpresion);
        this.cerrarModal();
        setTimeout(() => {
    ...
    //Obtener user Funcion en context api checar 
    obtenerUser = () => {
        const usuario = firebase.auth().currentUser.email
        const caca = firebase.auth().currentUser
        //console.log(caca)
        db.collection("usuarios").where("email", "==", usuario).get().then(querySnapshot => {
            ...
            this.setState({nombreVendedor: getUser})
        })
    }
    ...
    componentDidMount() {
        ...
        let usuario = null;
        const {hanldeVendedorChange} = this.context
        //console.log(this.context)
 
        db.collection("usuarios").where("email", "==", user).get().then(querySnapshot => {
            const data = querySnapshot.docs.map(doc => doc.data());
            const getUser = data[0].name
            const admin = data[0].rol
            
            console.log(getUser)
            this.context.handleVendedorChange(getUser)
    ...
    render() {
        const { rol, nombresClientes, nuevaClave, crearNuevo, nombresEmpresas, handleSelectUge, listaVisitador, handleCliente, obtDataCliente, handleEmpresa, getName, vendedor, uge, copia, oferta,
            ...
            // if(this.state.estadoImpresion){
            //     return(<OrdenPdf></OrdenPdf>)
            // }else 
            // if (this.state.estadoImpresion) {
            //     return <Redirect push to="/Orden/" />
            // }
            if (rol === "admin") {
            return (
                //Ceacion formulario
                <div id="OrdenCreada" className="div-form imprimir">
                {this.state.estadoImpresion ? <OrdenPdf></OrdenPdf> :
                                ...
                                <FormGroup row>
                                    ...
                                    <Col sm={3}>
                                        ...
                                        <Col >
                                            ...
                                            <Input type="select" name="visitador" value={visitador} onChange={handleChange}>
                                                <option value="">Selecciona</option>
                        ...
                        <fieldset className="fieldset">
                            <FormGroup row>
                                <Col sm={2}>
                                    <Label sm={12}  >CLAVE </Label>
                                    <Col>
                                        <Input type="text" readOnly placeholder={nuevaClave}></Input>
                                    </Col>
                                </Col>
                                <Col sm={3}>
                                    <Label sm={12} >UGE <span className="text-danger ">*</span>   </Label>
                                ...
                                <Col sm={2}>
                                    <Label sm={12}  >Vendedor Ejecutivo <span className="text-danger">*</span> </Label>
                                    <Col >
                                        <Input className="obligatorio" type="select" name="vendedor" value={vendedor} onChange={handleChange} required readOnly>
