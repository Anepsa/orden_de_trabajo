...
export const AppContext = React.createContext()
 ...
 export class AppContextProvider extends Component {
    ...
    this.getOrden = this.getOrden.bind(this)
   ...
   //Actualizacion del estado por cada cambio de valor
    ...
    handleVendedorChange = (name) => {
      this.setState({vendedor: name})
...
//Cambio de estado al selecionar un UGE en formulario crear orden
    handleSelectUge = (e) =>{
      ...
      if(this.state.rol == "admin"){
        ...
        db.collection("clientes").where("area", "==", ugeSelect).where("vendedor", "==" , this.state.getName)
        .get()
        .then(querySnapshot => {
          const dataNom = querySnapshot.docs.map(doc => doc.data().nombre);
          const dataEmp = querySnapshot.docs.map(doc => doc.data().empresa);  
  ...
  .then(querySnapshot =>{
    ...
    const data = querySnapshot.docs.map(doc => {
      ...
      if(data.length>0){
        ...
        pagos.push(pago);
        db.collection("pagos").doc(id).update({pagos:pagos}).then(updateSnapshot =>{
          this.getPagos(orden.clave);
        })
      }else{
        const nuevo = {
          pagos : [pago],
        ...
        db.collection("pagos").add(nuevo).then(updateSnapshot => {
          this.getPagos(orden.clave);
      });
    }
  })
...
// Mostrar una orden dando su productClave
getOrden(clave){
  db.collection("orden").where("productClave", "==", clave)
  .get()
  .then(querySnapshot =>{
...
//Editar orden de trabajo
  ...
  handleUpdateCliente=(e) =>{
    ...
    .then(function(querySnapshot) {
        querySnapshot.forEach(function(doc) {
            ...
            direccion:direccion, rfc:rfc, area:area, servicios:servicios});
        });
    })
   this.closeModal();
  }
  ...
  handleSubmitCliente =(e)=>{
    e.preventDefault();
    db.collection("clientes").get().then( snap =>{ // Obtener el total de documentos en la coleccion cliente.
      let contador = parseInt(snap.size) + 1 ;//Agregar 1 al contador
 ...
 //Enviar formulario orden Firebase
    handleSubmit = (e)  => {
        ...
        e.preventDefault();
            ...
            let obtDate=  new Date().toLocaleDateString("en-ZA");   
...
// obtener base de orden de trabajo
  obtenerBD=()=>{
    ...
    db.collection("orden").orderBy("date", "desc")
      ...
      db.collection("orden").where("vendedor", "==", obtName)
      .get()
      .then(querySnapshot => {
        const data = querySnapshot.docs.map(doc => doc.data());
  ...
  obtenerClientes=()=>{
    ...
    if(obtRol === "admin"){
            ...
            db.collection("clientes").orderBy("date", "desc")
              ...
              db.collection("clientes").where("vendedor", "==", obtName)
              .get()
              .then(querySnapshot => {
               const dataClientes =  querySnapshot.docs.map(doc => doc.data());
               const news = dataClientes[0].contador
...
//Obtener data modal orden 
    ...
    render() {
        ...
        visitadorNombre, rfcVisitador, direccionVisitador, delegacionVisitador, EDOVisitador, atencionVisitador, telVisitador, extTelVisitador,emailVisitador,
          ...
          dateToCompare, dataPagos, pagos, ordenPagos } = this.state;
      return (
        <AppContext.Provider
        value={{
          handleChangeCliente: this.handleChangeCliente,
          handleVendedorChange : this.handleVendedorChange,
